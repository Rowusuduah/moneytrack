'use strict';
/* ═══════════════════════════════════════════════════════════════
   MoneyTrack — Wealth tab. Private plan values live in this browser's
   localStorage and can be imported/exported as a local JSON file.
   Top-level code here must stay DOM-free and app.js-free: the pure
   functions are unit-tested in Node (tests/wealth.test.mjs).
   ═══════════════════════════════════════════════════════════════ */

const WL_PLAN_KEY = 'moneytrack_wealth_plan_v1';
const WL_EMPTY_PLAN = {
  netPerCheck: 0,
  payAnchor: '',
  savingsTargetMo: 0,
  ...
  returnPct: 5,
  years: 20,
  // IDs and transaction categories are public app structure; amounts and
  // editable labels are stored only in the user's browser.
  groups: [
    { id: 'giving', label: 'Giving', monthly: 0, color: '#a78bfa', bills: [
      { id: 'tithe', label: 'Tithe', monthly: 0, categories: ['Tithe'] },
      { id: 'offering', label: 'Offering', monthly: 0, categories: ['Offering'] }] },
    { id: 'housing', label: 'Housing', monthly: 0, color: '#60a5fa', bills: [
      { id: 'rent', label: 'Rent', monthly: 0, categories: ['Rent'] },
      { id: 'utilities', label: 'Utilities', monthly: 0, categories: ['Utilities'] }] },
    { id: 'transport', label: 'Transport', monthly: 0, color: '#2dd4bf', bills: [
      { id: 'carins', label: 'Car insurance', monthly: 0, categories: ['Car Insurance'] }] },
    { id: 'subs', label: 'Subscriptions', monthly: 0, color: '#f472b6', bills: [] },
    { id: 'protect', label: 'Protection & obligations', monthly: 0, color: '#fbbf24', bills: [] },
    { id: 'explore', label: 'Exploration', monthly: 0, color: '#fb923c', bills: [] },
    { id: 'ghana', label: 'Family support', monthly: 0, color: '#f87171', bills: [
      { id: 'remit', label: 'Remittance', monthly: 0, categories: ['Family Support (Ghana)'] }] },
    { id: 'annual', label: 'Annual irregular', monthly: 0, color: '#8a8aa6', bills: [] },
    { id: 'profdev', label: 'Professional development', monthly: 0, color: '#e879f9', bills: [] },
  ],
  livingColor: '#fbbf24',
  savingsColor: '#4ade80',
  milestones: [
    { id: 'starter', label: 'Starter buffer', amount: 0 },
    { id: 'goal2', label: 'Goal 2', amount: 0 },
    { id: 'ef', label: 'Emergency fund', amount: 0 },
    { id: 'goal4', label: 'Goal 4', amount: 0 },
  ],
};
let PLAN = JSON.parse(JSON.stringify(WL_EMPTY_PLAN));

function wlNumber(value, max) {
  const n = Number(value);
  if (!Number.isFinite(n) || n < 0 || n > max) throw new Error('Enter non-negative numbers within the allowed range.');
  return wlRound(n);
}

function wlNormalizePlan(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new Error('Invalid plan file.');
  const out = JSON.parse(JSON.stringify(WL_EMPTY_PLAN));
  out.netPerCheck = wlNumber(input.netPerCheck, 1000000);
  out.savingsTargetMo = wlNumber(input.savingsTargetMo, 1000000);
  out.kMo = wlNumber(input.kMo, 1000000);
  out.returnPct = wlNumber(input.returnPct, 100);
  out.years = wlNumber(input.years, 100);
  if (out.years < 1 || !Number.isInteger(out.years)) throw new Error('Projection years must be a whole number of at least 1.');
  const anchor = String(input.payAnchor || '');
  if (anchor && (!/^\d{4}-\d{2}-\d{2}$/.test(anchor) ||
      Number.isNaN(new Date(anchor + 'T00:00:00').getTime()) ||
      wlIsoLocal(new Date(anchor + 'T00:00:00')) !== anchor)) throw new Error('Enter a valid payday date.');
  out.payAnchor = anchor;
  if (!Array.isArray(input.groups) || input.groups.length !== out.groups.length) throw new Error('Invalid plan groups.');
  out.groups.forEach(g => {
    const source = input.groups.find(x => x && x.id === g.id);
    if (!source || !Array.isArray(source.bills) || source.bills.length !== g.bills.length) throw new Error('Invalid plan groups.');
    g.monthly = wlNumber(source.monthly, 1000000);
    g.label = String(source.label || g.label).slice(0, 80);
    g.bills.forEach(b => {
      const bill = source.bills.find(x => x && x.id === b.id);
      if (!bill) throw new Error('Invalid plan bills.');
      b.monthly = wlNumber(bill.monthly, 1000000);
      b.label = String(bill.label || b.label).slice(0, 80);
    });
  });
  if (!Array.isArray(input.milestones) || input.milestones.length !== out.milestones.length) throw new Error('Invalid milestones.');
  out.milestones.forEach((m, i) => {
    const source = input.milestones[i];
    if (!source) throw new Error('Invalid milestones.');
    m.amount = wlNumber(source.amount, 1000000000);
    m.label = String(source.label || m.label).slice(0, 80);
  });
  // Colors and transaction-category mappings stay fixed to trusted app values.
  return out;
}

function wlApplyPlan(input) { PLAN = wlNormalizePlan(input); return PLAN; }
function wlPlanConfigured() { return PLAN.netPerCheck > 0 && !!PLAN.payAnchor; }
function wlLoadStoredPlan() {
  try {
    if (typeof localStorage === 'undefined') return;
    const raw = localStorage.getItem(WL_PLAN_KEY);
    if (raw) wlApplyPlan(JSON.parse(raw));
  } catch { PLAN = JSON.parse(JSON.stringify(WL_EMPTY_PLAN)); }
}
wlLoadStoredPlan();
...
// Level of service for a given monthly daily-living amount.
...
// Reserve phase (cash at 4%) until the configured ladder is funded, then invest.
...
function wlConfigField(id, label, value, type) {
  const inputType = type || 'number';
  const attrs = inputType === 'number' ? ' min="0" step="0.01"' : '';
  return '<label class="wl-config-field"><span>' + escapeHTML(label) + '</span>' +
    '<input id="' + id + '" type="' + inputType + '"' + attrs +
    ' value="' + escapeHTML(String(value)) + '"></label>';
}

function renderWlConfig() {
  const el = document.getElementById('wl-config');
  if (!el) return;
  ...
  el.addEventListener('submit', e => {
    if (e.target.id !== 'wlc-form') return;
    e.preventDefault();
    try {
      const value = id => document.getElementById(id).value;
      const next = JSON.parse(JSON.stringify(PLAN));
      next.netPerCheck = value('wlc-pay');
      next.payAnchor = value('wlc-anchor');
      next.savingsTargetMo = value('wlc-save');
      next.kMo = value('wlc-match');
      next.returnPct = value('wlc-return');
      next.years = value('wlc-years');
      next.groups.forEach(g => {
        g.monthly = value('wlc-group-' + g.id);
        g.bills.forEach(b => { b.monthly = value('wlc-bill-' + b.id); });
      });
      next.milestones.forEach(m => {
        m.amount = value('wlc-ms-' + m.id);
        m.label = value('wlc-ms-label-' + m.id);
      });
      wlSavePrivatePlan(next);
    } catch (err) { wlConfigStatus(err.message); }
  });
  el.addEventListener('change', async e => {
    if (e.target.id !== 'wlc-file' || !e.target.files?.length) return;
    try {
      const file = e.target.files[0];
      if (file.size > 100000) throw new Error('Plan file is too large.');
      wlSavePrivatePlan(JSON.parse(await file.text()));
    } catch (err) { wlConfigStatus(err.message); }
  });
  el.addEventListener('click', e => {
    if (e.target.id !== 'wlc-export') return;
    const url = URL.createObjectURL(new Blob([JSON.stringify(PLAN, null, 2) + '\n'], { type: 'application/json' }));
    const a = document.createElement('a');
    a.href = url; a.download = 'moneytrack-private-plan.json'; a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  });
}

function wlConfigStatus(message) {
  const status = document.getElementById('wlc-status');
  if (status) status.textContent = message;
}

function wlSavePrivatePlan(input) {
  ...
  if (slider) {
    ...
    slider.value = String(PLAN.savingsTargetMo);
  }
  const ret = document.getElementById('wl-sl-ret');
  if (ret) ret.value = String(PLAN.returnPct);
  const yrs = document.getElementById('wl-sl-yrs');
  if (yrs) yrs.value = String(PLAN.years);
  renderWealthTab();
  wlConfigStatus('Plan saved on this device.');
}
...
function renderWealthTab() {
  ...
  if (!wlPlanConfigured()) {
    ...
    if (sub) sub.textContent = 'Set up your private plan to use the Wealth dashboard.';
    return;
  }
  const slider = document.getElementById('wl-sl-save');
  if (slider && !slider._wlInitialized) {
    slider.max = String(Math.max(5000, Math.ceil(PLAN.savingsTargetMo * 1.5 / 25) * 25));
    slider.value = String(PLAN.savingsTargetMo);
    document.getElementById('wl-sl-ret').value = String(PLAN.returnPct);
    document.getElementById('wl-sl-yrs').value = String(PLAN.years);
    slider._wlInitialized = true;
  }
  const txns   = loadTxns();
...
function renderWlStudio() {
  ...
  for (let y = 0; y <= yrs; y += step) {
    g += '<text x="' + X(y) + '" y="' + (H - 7) + '" fill="currentColor" opacity="0.55" ' +
      'font-size="9" text-anchor="middle">' + y + '</text>';
  }
  let d = 'M' + X(0) + ',' + Y(0);
