...
const CATEGORY_COLORS = {
  ...
  'Rent':            '#f87171', 'Utilities':     '#fb923c', 'Insurance':   '#fb923c', 'Laundry': '#fb923c',
  'Groceries':       '#fbbf24', 'Dining Out':    '#fbbf24', 'Fast Food':   '#fbbf24', 'Food Delivery': '#fbbf24',
  'Snacks & Drinks': '#fbbf24', 'Coffee':        '#fbbf24',
  'Gas':             '#60a5fa', 'Rideshare':     '#60a5fa', 'Car Insurance': '#60a5fa', 'Parking': '#60a5fa',
  'Travel':          '#60a5fa',
  'Medical':         '#f472b6', 'Pharmacy':      '#f472b6', 'Gym':         '#f472b6',
  'Clothing':        '#a78bfa', 'Shoes & Accessories': '#a78bfa', 'Electronics': '#a78bfa',
  'Home & Furniture':'#a78bfa', 'Household Essentials': '#a78bfa', 'Beauty & Grooming': '#a78bfa',
  'School Supplies': '#a78bfa', 'Amazon':        '#a78bfa', 'Online Shopping': '#a78bfa',
...
// ─── Savings Goals ────────────────────────────────────────────────
// Average monthly net money moved INTO `accountIds` over the last `n` completed
// calendar months, counting only months that had activity in those accounts.
// This is what actually grows a savings goal's tracked balances — transfers and
// income landing in the set, minus transfers/expenses leaving it. It replaces
// projecting from overall net income, which wrongly assumed every dollar not
// spent reaches savings when most of it stays in checking. `refDate` is
// injectable for tests. Returns 0 when there is no contribution history.
function avgMonthlyContribution(txns, accountIds, n, refDate) {
  const today = refDate || new Date();
  const base = today.getFullYear() * 12 + today.getMonth();
  const first = base - n, last = base - 1;            // the last n completed months
  ...
  for (const t of txns) {
    ...
    let delta;
    if (t.type === 'transfer') {
      const inSet = ids.has(t.toAccount), outSet = ids.has(t.account);
      if (!inSet && !outSet) continue;                // transfer unrelated to the set
      delta = (inSet ? amt : 0) - (outSet ? amt : 0); // internal moves net to 0
    } else if (t.type === 'income' && ids.has(t.account)) {
      delta = amt;                                     // income deposited into the set
    } else if (t.type === 'expense' && ids.has(t.account)) {
      delta = -amt;                                    // spent straight from the set
    } else {
...
function renderSavingsGoals() {
  ...
  if (!goals.length) {
    ...
    el.innerHTML = goals.map(g => {
      ...
      // Project from money actually moved into THIS goal's tracked accounts
      // over the last 3 months — not overall net income, most of which stays in
      // checking and never reaches the goal.
      const pace = avgMonthlyContribution(txns, trackIds, 3, today);
      let projection = '';
      if (current >= g.target) {
        projection = 'Goal reached! 🎉';
      } else if (pace > 0) {
        const monthsLeft = Math.ceil((g.target - current) / pace);
        if (monthsLeft <= 120) {
          ...
          projection = '10+ yrs at this pace';
        }
      }
